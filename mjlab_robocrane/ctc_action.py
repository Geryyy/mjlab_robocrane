...
with a computed-torque style law:
  tau = M(q) * (qddot_ff + Kp (q_d - q) + Kd (qdot_d - qdot)) + bias
where M(q) is the full inertia matrix computed via inverse dynamics and bias
includes Coriolis, gravity, and other passive forces from MuJoCo.
"""
...
class JointAccelerationCtcAction(ActionTerm):
    ...
    def __init__(self, cfg: JointAccelerationCtcActionCfg, env):
        ...
        self._ee_site_id = int(
            self._entity.indexing.site_ids[self._ee_site_local_id].item()
        )
        ...
        self._force_norm = torch.zeros(
            (self.num_envs,), dtype=torch.float32, device=self.device
        )
        self._tau_res_norm = torch.zeros(
            (self.num_envs,), dtype=torch.float32, device=self.device
        )
    ...
    def apply_actions(self) -> None:
        ...
        # Compute control torque using full inertia matrix via inverse dynamics:
        # tau = M(q) * qddot_ref + bias
        # This uses mjwarp.inverse() which computes the full model-based torque efficiently
        tau = self._compute_model_torque(qddot_ref)
        ...
        # Compute nominal torque (feedforward only, without feedback terms)
        self._tau_nominal[:] = self._compute_model_torque(self._qddot_cmd)
    ...
    def _compute_model_torque(self, qddot_des: torch.Tensor) -> torch.Tensor:
        """Compute model-based torque using full inertia matrix via inverse dynamics.

        Computes tau = M(q) * qddot_des + bias, where M(q) is the full inertia matrix
        and bias includes Coriolis, gravity, and other passive forces.

        Args:
          qddot_des: Desired joint accelerations (num_envs, n_joints)

        Returns:
          tau_model: Model-based torques (num_envs, n_joints)
        """
    ...
    def _estimate_external_wrench(self, tau_res: torch.Tensor) -> torch.Tensor:
        ...
        eye = torch.eye(
            self._action_dim, device=self.device, dtype=jac6.dtype
        ).unsqueeze(0)
