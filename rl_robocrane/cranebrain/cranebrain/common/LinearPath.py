...
class LinearPath: 
    ...
    def plan(self, q_start, q_end):
        ...
        via_pts = (np.linspace(q_start, q_end, self.n_ctrl_pts))
        # compute bspline
        self.ctr_pts, self.knot_vec = bs.compute_control_points(via_pts, self.k)
        np.set_printoptions(precision=2, suppress=True)
        print("via_pts: \n", via_pts)
        print("ctr pts: \n", self.ctr_pts)
        # self.ctr_pts = via_pts
        return True
    ...
    def evaluate(self, u, spline=None):
        ...
        return bs.bspline(u, self.knot_vec, self.ctr_pts, self.k)
    ...
    def get_path(self, n_path_pts):
        ...
        for u in u_vec:
            q = self.evaluate(u)
            print("q({:.2f}): {}".format(u, np.array2string(q, precision=2, separator=', ')))
...
def main():
    ...
    path = LinearPath(pin_model, pin_data, tool_frame_id, xml_path)
    ...
    q_sol.append(path.inverse_kinematics(H_right, q_sol[0]))
